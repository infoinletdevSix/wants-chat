      ...
      <div className="p-6 space-y-6">
        {isPrefilled && (
          <div className="flex items-center gap-2 px-4 py-2 bg-primary-500/10 rounded-xl border border-primary-500/20">
            ...
            <span className="text-sm text-primary-500 font-medium">{t('tools.wholesalePriceCalculator.valuesLoadedFromAiResponse', 'Values loaded from AI response')}</span>
          </div>
        )}
          ...
          <div className="flex flex-wrap gap-2">
            {discountTiers.map((tier) => (
              ...
              >
                ...
                <div className={`text-xs ${parseFloat(wholesaleDiscount) === tier.value ? 'text-primary-100' : isDark ? 'text-gray-500' : 'text-gray-500'}`}>
                  {tier.tier}
                </div>
              </button>
            ))}
